"""Standalone router stress test runner with JSON output.

Discovers a BACnet server on a remote network through a router, then
runs mixed-workload stress workers.  All traffic is routed through the
BACnet router, measuring cross-network routing throughput and latency.
...
Configuration via env vars:
    ...
    DISCOVERY_TIMEOUT:   Route/device discovery timeout (default: 30)
"""
...
async def main() -> None:
    ...
    total_workers = num_pools * per_pool + objlist_workers + 1  # +1 for route check

    print(
        f"Router Stress test: {num_pools} pools x "
        f"({readers_per_pool}R + {writers_per_pool}W + "
        f"{rpm_per_pool}RPM + {wpm_per_pool}WPM) + "
        f"{objlist_workers}OL + 1 route-check = "
        f"{total_workers} workers via network {remote_network}",
        file=sys.stderr,
    )
    print(
    ...
    async with contextlib.AsyncExitStack() as stack:
        ...
        print(
            f"\nPASSED: {throughput:.1f} req/s sustained (routed), "
